...
//  COMPELTED (19) Create constant int values representing each column name's position above
...
//  COMPLETED (20) Create a constant int to identify our loader used in DetailActivity
    public static final int ID_DETAIL_LOADER = 4242;
...
//      COMPLETED (13) Find each of the TextViews by ID
        mDateDisplay = (TextView) findViewById(R.id.tv_selected_date);
        mDescriptionDisplay = (TextView) findViewById(R.id.tv_selected_description);
        mHighDisplay = (TextView) findViewById(R.id.tv_selected_high_temperature);
        mLowDisplay = (TextView) findViewById(R.id.tv_selected_low_temperature);
        mHumidityDisplay = (TextView) findViewById(R.id.tv_selected_humidity);
        mWindDisplay = (TextView) findViewById(R.id.tv_selected_wind);
        mPressureDisplay = (TextView) findViewById(R.id.tv_selected_pressure);
...
//      COMPLETED (26) Display a readable data string
        mDateDisplay.setText(SunshineDateUtils.getFriendlyDateString(this, data.getLong(INDEX_WEATHER_DATE), true));
//      COMPLETED (27) Display the weather description (using SunshineWeatherUtils)
        mDescriptionDisplay.setText(SunshineWeatherUtils.getStringForWeatherCondition(this, data.getInt(INDEX_WEATHER_CONDITION_ID)));
//      COMPLETED (28) Display the high temperature
        mHighDisplay.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(INDEX_WEATHER_MAX_TEMP)));
//      COMPLETED (29) Display the low temperature
        mLowDisplay.setText(SunshineWeatherUtils.formatTemperature(this, data.getDouble(INDEX_WEATHER_MIN_TEMP)));
//      COMPLETED (30) Display the humidity
        mHumidityDisplay.setText(String.valueOf(data.getFloat(INDEX_WEATHER_HUMIDITY)) + " %");
//      COMPELTED (31) Display the wind speed and direction
        mWindDisplay.setText(SunshineWeatherUtils.getFormattedWind(this, data.getFloat(INDEX_WEATHER_WIND_SPEED), data.getFloat(INDEX_WEATHER_DEGREES)));
//      COMPLETED (32) Display the pressure
        mPressureDisplay.setText(String.valueOf(data.getFloat(INDEX_WEATHER_PRESSURE)));
//      COMPLETED (33) Store a forecast summary in mForecastSummary
        mForecastSummary = mDescriptionDisplay.getText().toString();
    }
