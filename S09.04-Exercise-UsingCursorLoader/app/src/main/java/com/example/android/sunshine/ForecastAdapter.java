...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        int weatherConditionId = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
        int highTempIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
        int lowTempIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);

        String date = mCursor.getString(dateIndex);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, mCursor.getInt(weatherConditionId));
        double hightTemp = mCursor.getDouble(highTempIndex);
        double lowTemp = mCursor.getDouble(lowTempIndex);

        String highLowTemps = SunshineWeatherUtils.formatHighLows(mContext, hightTemp, lowTemp);

        String niceDate = SunshineDateUtils.getFriendlyDateString(mContext, Long.parseLong(date), true);

        String weatherForThisDay = niceDate + " - " + description + highLowTemps;
